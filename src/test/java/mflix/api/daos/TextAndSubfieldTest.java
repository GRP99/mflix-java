...
public class TextAndSubfieldTest extends TicketTest {
    ...
    public void testMultipleGenreSearch() {
        ...
        int limit = 25;
        int skip = 0;
        String[] array = genres.toArray(new String[0]);
        Iterable<Document> movies = dao.getMoviesByGenre(sortKey, limit, skip, array);
        int numMovies = 0;
        for (Document ignored : movies) {
            numMovies++;
        }

        Assert.assertTrue("getMoviesByGenre should be returning documents",
                numMovies > 0);
        Assert.assertEquals(expectedCount, dao.getGenresSearchCount(array));
    }
