...
public class FacetedSearchTest extends TicketTest {
    ...
    public void testRatingRuntimeBuckets() {
        ...
        ArrayList<Document> rating = (ArrayList<Document>) moviesInfo.get(0).get("rating");
        assertEquals(
                "Check your $bucket rating sub-pipeline on getMoviesFaceted() for multiple cast in single cast",
                3,
                rating.size());

        ArrayList<Document> runtime = (ArrayList<Document>) moviesInfo.get(0).get("runtime");
        assertEquals(
    ...
    public void testFacetedSearchPaging() {
        ...
        ArrayList<Document> movies = (ArrayList<Document>) moviesInfo.get(0).get("movies");
        assertEquals(
