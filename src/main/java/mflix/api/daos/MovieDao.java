...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
        //DO> Ticket: Handling Errors - implement a way to catch a any potential exceptions thrown while validating a movie id.
        try {
            Document movieById = moviesCollection.find(new Document("_id", new ObjectId(movieId))).first();
            if (movieById != null && movieById.isEmpty()) {
    ...
    public Document getMovie(String movieId) {
        //DO> Ticket: Get Comments - implement the lookup stage that allows the comments to retrieve with Movies.
    ...
    public List<Document> getMoviesByCountry(String... country) {
        //DO> Ticket: Projection - implement the query and projection required by the unit test
     ...
     * db.movies.find({"$text": { "$search": `keywords` }}, {"score": {"$meta": "textScore"}}).sort({"score": {"$meta": "textScore"}})
     *
    ...
    public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
        //DO> Ticket: Subfield Text Search - implement the expected cast filter and sort
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
        List<Document> movies = new ArrayList<>();
        //DO > Ticket: Paging - implement the necessary cursor methods to support simple pagination like skip and limit in the code below
        moviesCollection.find(Filters.in("genres", genres)).sort(Sorts.descending(sortKey)).limit(limit).skip(skip).iterator().forEachRemaining(movies::add);
        return movies;
    }
    ...
    public List<Document> getMoviesCastFaceted(int limit, int skip, String... cast) {
        ...
        // DO > Ticket: Faceted Search - build the aggregation pipeline by adding all stages in the correct order.
    ...
    private Bson buildFacetStage() {
        return Aggregates.facet(new Facet("runtime", buildRuntimeBucketStage()),
                ...
                new Facet("movies", Aggregates.addFields(
